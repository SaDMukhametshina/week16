...
function makeFour() {
	...
	for (let i = 0; i < forms4.length; i++) {
		formNames[i] = forms4[i].getAttribute('name');
	}
...
function makeSeven() {
	let form2 = document.forms.formTwo.elements;
	elementsList = ``;
	for (i = 0; i < form2.length; i++) {
		elementsList += ` - ${form2[i].getAttribute('name')}`;
	}
...
function makeEight() {
	let form1 = document.forms.formOne.elements;
	elementsList2 = ``;
	for (i = 0; i < form1.length; i++) {
		elementsList2 += ` - ${form1[i].getAttribute('name')}`;
	}
	paragraphEight.textContent = elementsList2;
}
...
function makeNine() {
	let form3 = document.forms.formThree.elements;
	elementsList3 = ``;
	for (i = 0; i < form3.length; i++) {
		elementsList3 += ` - ${form3[i].getAttribute('name')}`;
	}
...
function makeTen() {
	form4 = document.forms.lastForm.elements;
	paragraphTen.textContent = form4.fourthName.value;
}
...
function makeEleven() {
	let formSelect1 = document.forms.formOne.elements.firstSelect;
	optionsValues = ``;
	for (let i = 0; i < formSelect1.length; i++) {
		optionsValues += ` ${formSelect1[i].value}`;
	}
...
function makeTwelve() {
	let form2 = document.forms.formTwo.elements;
	let checkbox1 = form2.checkboxOne.getAttribute('id');
	let checkbox2 = form2.checkboxTwo.getAttribute('id');
	let checkbox3 = form2.checkboxThree.getAttribute('id');
...
function checkButton() {
	const form4 = document.forms.lastForm.elements.fourthName;
	if (form4.checked) {
...
function checkOption() {
	const form1 = document.forms.formOne.elements.firstSelect;
	if (form1.value == 'Опция 1') {
		paragraphFourteen.textContent = "Выбран первый вариант";
	}
	if (form1.value == 'Опция 2') {
		paragraphFourteen.textContent = "Выбран второй вариант";
	}
	if (form1.value == 'Опция 3') {
...
function makeSixteen() {
	const form2 = document.forms.formTwo.elements.checkboxThree;
	form2.checked = true;
}
...
const formOneError = document.getElementById('errorMessage')

formOne.addEventListener('submit', function (event) {
	if (formOne.elements.firstName.value == "") {
		formOneError.textContent = 'Необходимо заполнить все поля';
		event.preventDefault();
	}
	if (formOne.elements.firstEmail.value == "") {
		formOneError.textContent = 'Необходимо заполнить все поля';
		event.preventDefault();
	}
});
...
emailInput = document.forms.formOne.elements.firstEmail;
const errorMessage = document.getElementById('errorMessage');

emailInput.oninput = function () {
	let RegExp = /^[A-Z0-9._%+-]+@[A-Z0-9-]+.+.[A-Z]{2,4}$/i;
	if (RegExp.test(emailInput) == false) {
		errorMessage.textContent = "Проверьте правильность ввода Email"
	}
};
...
document.querySelector('.b-21').onclick = function (event) {
	...
	if (!isChecked) {
		event.preventDefault();
		document.getElementById('result21').textContent = 'Выберите вариант ответа';
	} else {
...
const lastForm1 = document.forms.lastForm;
lastForm1.addEventListener('submit', (event) => {
	event.preventDefault();
	const submitButton = lastForm.elements.fourthButton;
	if (lastForm1.elements.fourthName.checked) {
		submitButton.disabled = false;
		document.getElementById('result23').textContent = 'Кнопка разблокирована';
	} else {
		submitButton.disabled = true;
		document.getElementById('result23').textContent = 'Кнопка заблокирована';
	}
});
...
document.querySelector('.b-26').onclick = function (event) {
	...
	result26.textContent = thirdName.getAttribute('placeholder');
};
...
function handleSubmit(event) {
	...
	console.log(selectedOptions, formTwo.elements.secondEmail.value, formTwo.elements.secondName.value);
}
